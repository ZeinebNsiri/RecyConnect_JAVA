...
public class EventAddController {
    ...
    private void navigateToEventList() {
        try {
            ...
            if (contentPane != null) {
                contentPane.getChildren().setAll(listView); // Update contentPane
            } else {
                // Fallback: Open in a new window if contentPane not set
                Stage stage = (Stage) nameField.getScene().getWindow();
                stage.setScene(new Scene(listView));
            }
        } catch (IOException e) {
            e.printStackTrace();
            showAlert(Alert.AlertType.ERROR, "Error", "Failed to return to event list: " + e.getMessage());
        }
    }
    private StackPane contentPane; // Store the reference
