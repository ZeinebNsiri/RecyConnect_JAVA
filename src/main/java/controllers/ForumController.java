...
public class ForumController {
    ...
    private void handleCommentaireClick(Post post) {
        try {
            // Load the BaseUser.fxml into the current scene
            FXMLLoader baseUserLoader = new FXMLLoader(getClass().getResource("/BaseUser.fxml"));
            Parent baseUserRoot = baseUserLoader.load();

            // Get the controller for BaseUser and set it in the current scene
            BaseUserController baseUserController = baseUserLoader.getController();

            // Load the PostDetail.fxml into the BaseUser layout
            FXMLLoader postDetailLoader = new FXMLLoader(getClass().getResource("/PostDetail.fxml"));
            Parent postDetailRoot = postDetailLoader.load();

            // Get the controller for PostDetail and set the post
            PostDetailController postDetailController = postDetailLoader.getController();
            postDetailController.setPost(post); // <-- Pass the selected post to the controller

            // Set the PostDetail view into the BaseUser layout (assuming you have a container like BorderPane or VBox)
            baseUserController.setCenterContent(postDetailRoot); // Make sure BaseUserController has a method for this

            // Set the BaseUser content to the scene
            savedToggle.getScene().setRoot(baseUserRoot);
    ...
    private void handleCreerPublication(ActionEvent event) {
        try {
            ...
            baseUserController.AddPosts();

            myPostsToggle.getScene().setRoot(root);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
